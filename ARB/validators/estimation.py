...
class EstimationValidator:
    ...
    def search_best_scores_once(self, apa_fn, params_range_kw: dict, dataset_name) -> list:
        scores = []
        ...
        torch.save(best_x_hat, './out/'+ dataset_name +algo_name + 'feature.pt')
        return scores
    ...
    def run(file_name="iter_le_30",
            dataset_names=['cora', 'citeseer', 'computers', 'photo', 'steam', 'pubmed', 'cs', 'arxiv', 'product'],
            run_algos=['fp', 'pr', 'ppr', 'mtp', 'arb', 'arb2'], max_num_iter=30, only_val_once=True,
        ...
        for seed in range(1):
            ...
            for dataset_name in dataset_names:
                ...
                algos = {
                    ...
                    'arb': (apa.arb, {"alpha": (0.0, 1.0), "beta": (0.0, 1.0)}),
                    'arb2': (apa.arb2, {"alpha": (0.0, 1.0), "beta": (0.0, 1.0), "gamma": (0.0, 1.0)})
                }
                for algo_name in run_algos:
                    if only_val_once:
                        ...
                        if not EstimationValidator.is_executed(scores, seed_idx=seed, dataset_name=dataset_name,
                            ...
                            for row, col, value in v.search_best_scores_once(algos[algo_name][0], algos[algo_name][1],dataset_name):
