...
from ..models.apa import APA, arbLoss, arbwithParams
...
class SGDValidator:
    ...
    def compare(dataset_name, apa: APA, val_nodes, params, metric, k, epochs):
        ...
        arb = arbLoss(edge_index, x_all, trn_nodes, is_binary=apa.is_binary, **params)

        # analytical_solution
        x_hat = apa.arb_analytical_solution(**params)
        ...
        if not s.has_value("sgd_mse", dataset_name, epochs - 1):
            # sgd
            optimizer = torch.optim.Adam(arb.parameters(), lr=0.01)
            for epoch in range(epochs):
                optimizer.zero_grad()
                loss = arb()
                loss.backward()
                optimizer.step()
                score = calc_single_score(dataset_name, arb.get_out(), x_all, val_nodes, metric, k)
                mse = torch.nn.functional.mse_loss(x_hat, arb.get_out(), reduction='sum')
        ...
        for seed in range(1):
            ...
            for dataset_name in dataset_names:
                ...
                def fn(point):
                    ...
                    arb = arbLoss(data.edges, data.x, data.trn_mask, is_binary=data.is_binary, **params)
                    optimizer = torch.optim.Adam(arb.parameters(), lr=0.01)
                    best_score = None
                    for epoch in range(epochs):
                        optimizer.zero_grad()
                        ...
                        loss.backward()
                        optimizer.step()
                        score = calc_single_score(dataset_name, arb.get_out(), data.x, data.val_mask, metric, k)
                    ...
                    return best_score, (arb.get_out(), epoch)
        ...
        for seed in range(1):
            ...
            for dataset_name in dataset_names:
                ...
                for epoch in range(epochs):
                    ...
                    x_hat = apa.arb(alpha=alpha1, beta=beta1, num_iter=30)
        ...
        for seed in range(1):
            ...
            for dataset_name in dataset_names:
                ...
                edge_index, x_all, y_all, trn_nodes, val_nodes, test_nodes, num_classes = d.load_data(dataset_name,
                                                                                                      ...
                                                                                                      0.4, 0.1, 0.5),
                                                                                                      seed=seed)
                arb = arbwithParams(x_all, y_all, edge_index, trn_nodes, d.is_binary(dataset_name))

                loss_fn = nn.MSELoss()
                optimizer = torch.optim.Adam(arb.parameters(), lr=0.05)
                best_score = None
                test_score = None
